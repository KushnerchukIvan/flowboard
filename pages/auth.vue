...
<template>
  <main class="auth-page">
    <NuxtLink to="/" class="brand auth-brand"
    ...
    <section class="auth-card">
      ...
      <p class="welcome-sub">
        {{
          mode === "register"
            ? "Зареєструйтеся, щоб організувати роботу команди."
            : "Увійдіть, щоб продовжити роботу над проєктами."
        }}
      </p>
      <form class="auth-form" @submit.prevent="submit">
        <label v-if="mode === 'register'"
          >Ваше ім’я<input
            ...
            required
            minlength="2"
            placeholder="Анна Мельник"
        /></label>
        <label
          >Електронна пошта<input
            v-model="email"
            type="email"
            autocomplete="email"
            required
      ...
      <p class="auth-switch">
        {{ mode === "register" ? "Вже маєте акаунт?" : "Ще не маєте акаунта?" }}
        <button
          type="button"
          @click="
      ...
      <p class="auth-security">
        JWT сесія зберігається в HttpOnly cookie. Паролі захищені хешуванням.
      </p>
    </section>
