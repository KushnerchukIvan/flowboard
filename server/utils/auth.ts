...
const globalUsers = globalThis as typeof globalThis & {
  __flowboardUsers?: Map<string, UserRecord>;
  __flowboardUsersLoaded?: boolean;
};
export const users = (globalUsers.__flowboardUsers ??= new Map<
  string,
  UserRecord
>());
...
export async function saveUser(user: UserRecord) {
  ...
  await writeFile(temporaryFile, JSON.stringify([...users.values()], null, 2), {
    mode: 0o600,
  });
...
function secret(event: H3Event) {
  ...
  if (process.env.NODE_ENV === "production")
    throw createError({
      statusCode: 500,
      statusMessage: "JWT_SECRET must be configured",
    });
  return "flowboard-local-development-secret-change-before-deploying";
}

export function assertAuthConfigured(event: H3Event) {
  secret(event);
}
...
export function issueSession(event: H3Event, user: UserRecord) {
  ...
  const signature = createHmac("sha256", secret(event))
    .update(unsigned)
    .digest("base64url");
  setCookie(event, cookieName, `${unsigned}.${signature}`, {
    httpOnly: true,
...
export function clearFlowboardSession(event: H3Event) {
...
export function getSessionUser(event: H3Event): SessionUser | null {
  ...
  try {
    ...
    const expected = createHmac("sha256", secret(event))
