...
export default defineEventHandler(async (event) => {
  ...
  if (!name || name.length < 2)
    throw createError({
      statusCode: 400,
      statusMessage: "Вкажіть ім’я (мінімум 2 символи)",
    });
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
    throw createError({
      statusCode: 400,
      statusMessage: "Вкажіть коректну електронну адресу",
    });
  if (password.length < 8)
    throw createError({
      statusCode: 400,
      statusMessage: "Пароль має містити щонайменше 8 символів",
    });
  ...
  if (!(await saveUser(user)))
    throw createError({
      statusCode: 409,
      statusMessage: "Акаунт із цією поштою вже існує",
    });
