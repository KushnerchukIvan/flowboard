...
describe('projects store', () => {
  it('filters projects by name and client', () => {
    ...
    store.projects = [{ id: 'p1', name: 'Банкінг', client: 'Northstar', color: 'violet', progress: 72, due: '18 жов', members: [], tasks: 3 }]
    store.search = 'north'
  ...
  it('moves tasks across workflow statuses', () => {
    ...
    expect(store.tasks.find((task) => task.id === 't1')?.status).toBe('progress')
  })
})
